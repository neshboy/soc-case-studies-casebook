...
const REPO_ROOT = "C:/Users/User/projects/soc-case-studies-casebook";
...
// BOOK-INDEX.md for this book is one big "Case Table" (not "Part Table"/"Appendix Table"
// like the Detection Engineering Handbook and SOC Manager's Operating Handbook) grouped by
// "### Section X — Name" subheadings (Identity, Endpoint, Network, Web/Email, Cloud, Insider,
// Ambiguous/False-Positive). It also has an "## Appendix Table" further down, but that table is
// explicitly "proposed, not yet drafted" (BOOK-INDEX.md's own words) — no appendix files exist
// yet, and that table's third column ("Contents") is prose containing its own backtick-quoted
// filenames (e.g. `` `STYLE-GUIDE.md` ``, `` `CASE-INVENTORY.md` ``) that would be misread as
// chapter file paths by a naive parser. So: parse rows only while inside the Case Table section,
// and stop dead the moment the Appendix Table heading is seen.
function parseIndex() {
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith("## Appendix Table")) {
      inAppendix = true;
      continue;
    }
    if (line.startsWith("## Case Table")) {
      inAppendix = false;
      continue;
    }
    ...
    if (line.startsWith("|")) {
      ...
      // split, then restore (same escaped-pipe-safe logic as the DEH/SOC Manager build_book.js).
      ...
      const id = cells[0];
      if (!/^CB-[0-9]+$/.test(id)) continue; // header/separator row, or a non-case table entirely
      const title = cells[1];
      const pathMatch = cells[2].match(/`([^`]+)`/);
      if (!pathMatch) continue;
      // Cells already read e.g. `cases\cb01-the-spray-that-almost-worked.md` - resolves into
      // cases/, not chapters/, since this book's unit of content is a case, not a chapter.
      const target = pathMatch[1].replace(/\\/g, "/");
      const label = `Case ${id} — ${title}`;
...
// This book's six content tags (STYLE-GUIDE.md §11) are organized around stage of
// investigation, not reader role - different from both companion volumes' tag sets.
const TAG_COLORS = {
  CONCEPT: "tag-concept",
  ANALYST: "tag-analyst",
  PIVOT: "tag-pivot",
  HYPOTHESIS: "tag-hypothesis",
  ESCALATION: "tag-escalation",
...
function main() {
  ...
  :root {
    ...
    --blue-tint: #cde2fb; --orange-tint: #f9d9c4; --aqua-tint: #bdeedb; --yellow-tint: #fce8bf; --violet-tint: #e3ddf7; --magenta-tint: #fadce8; --green-tint: #cdeccd; --red-tint: #f9d4d3;
  }
  ...
  * { box-sizing: border-box; }
  ...
  .tag-pivot { background: var(--blue-tint); color: #164a8a; border: 0.5pt solid var(--blue); }
  .tag-hypothesis { background: var(--violet-tint); color: #2e2470; border: 0.5pt solid var(--violet); }
  .tag-escalation { background: var(--orange-tint); color: #93401d; border: 0.5pt solid var(--orange); }
  .tag-lesson-learned { background: var(--green-tint); color: #0b5c0b; border: 0.5pt solid var(--green); }
  `;
...
<head>
<meta charset="utf-8">
<title>The SOC Case Studies Casebook</title>
<style>${css}</style>
</head>
<body>
<section class="cover">
  <div class="kicker">NESHBOY SOC Professional Library</div>
  <h1>THE SOC CASE STUDIES CASEBOOK</h1>
  <div class="accent-rule"></div>
  <h2>Nineteen Full, End-to-End Synthetic SOC Investigation Narratives — Alert to Closure, Told Once, Start to Finish</h2>
